...
const IEMM_SOURCE_TEMPLATES = [
  ["iemm-qd-001", "Quyết định cá biệt IEMM", "/templates/iemm/01-quyet-dinh-ca-biet.docx"],
  ["iemm-qd-ban-hanh-001", "Quyết định ban hành hoặc phê duyệt văn bản IEMM", "/templates/iemm/02-quyet-dinh-ban-hanh-van-ban.docx"],
  ["iemm-quy-che-001", "Quy chế, quy định IEMM", "/templates/iemm/03-quy-che-quy-dinh.docx"],
  ["iemm-van-ban-kem-quyet-dinh-001", "Văn bản ban hành kèm theo quyết định IEMM", "/templates/iemm/04-van-ban-ban-hanh-kem-theo-quyet-dinh.docx"],
  ["iemm-cv-001", "Công văn hành chính IEMM", "/templates/iemm/05-cong-van-hanh-chinh.docx"],
  ["iemm-tb-001", "Thông báo nội bộ IEMM", "/templates/iemm/06-thong-bao-noi-bo-vien.docx"],
  ...
  ["iemm-bb-001", "Biên bản IEMM", "/templates/iemm/09-bien-ban.docx"],
  ["iemm-van-ban-co-ten-loai-001", "Văn bản hành chính chung IEMM", "/templates/iemm/10-van-ban-chung.docx"],
  ["iemm-ban-sao-001", "Bản sao văn bản IEMM", "/templates/iemm/11-ban-sao-van-ban.docx"],
  ["iemm-thu-moi-001", "Thư mời họp IEMM", "/templates/iemm/12-thu-moi-hop.docx"],
  ["iemm-thu-hoan-hop-001", "Thư báo hoãn họp IEMM", "/templates/iemm/13-thu-bao-hoan-hop.docx"],
  ["iemm-cong-van-dinh-chinh-001", "Công văn đính chính văn bản IEMM", "/templates/iemm/14-cong-van-dinh-chinh.docx"],
] as const;
...
test("IEMM catalog includes the complete source-backed administrative template set", () => {
  for (const [id, name, sourcePath] of IEMM_SOURCE_TEMPLATES) {
    ...
    assert.ok(record.referenceSources?.some((source) => /Phụ lục VII|Mẫu văn bản hành chính TVCI/i.test(source)));
  }
});
...
test("source-backed IEMM DOCX templates use IEMM footer branding", () => {
  for (const [, , sourcePath] of IEMM_SOURCE_TEMPLATES) {
    ...
    const footerXml = zip.getEntries()
      ...
      .join("\n");
    assert.match(footerXml, /Mẫu văn bản IEMM/);
    assert.doesNotMatch(footerXml, /Mẫu văn bản TVCI/);
  }
});
